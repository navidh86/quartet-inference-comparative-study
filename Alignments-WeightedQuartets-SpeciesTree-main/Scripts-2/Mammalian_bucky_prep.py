...
wqrts_output_folder = f"{mammalian_folder}/Output"
...
def prepare_nexus_sequences():
    ...
    output_folder_rep = os.path.join(output_folder, f"all_nexus")
    os.makedirs(output_folder_rep, exist_ok=True)

    for gene in genes:
        input_file_sequences_fasta = os.path.join(input_folder, f"{gene}/{gene}.fasta")
...
def run_mr_bayes():
    ...
    for gene in genes[start:end+1]:
        start_time = time.time()
        cmd = f"mb mb{gene}.nex >> {gene}.log"
        print(cmd)
        os.system(cmd)

        os.remove(f"{gene}.run1.p")
        os.remove(f"{gene}.run2.p")
        os.remove(f"{gene}.mcmc")
        os.remove(f"{gene}.ckp")
        # os.remove(f"{gene}.log")

        with open("time_mb.txt", "a") as fp:
            fp.write(f"Took {time.time()-start_time} seconds for gene {gene}\n")
...
def run_mbsum():
    ...
    curdir = os.getcwd()
    input_folder_rep = os.path.join(output_folder, f"all_nexus")
    output_folder_rep = os.path.join(output_folder, f"all_in")
    ...
    for gene in genes:
        ...
        ## not deleting, as they are very time consuming to reproduce
...
def run_bucky():
    ...
    input_folder_rep = os.path.join(output_folder, f"{gt_type}")
    ...
    with open("time_bukcy.txt", "a") as fp:
        fp.write(f"BUCKy took {time.time()-start_time} seconds \n")


mapping_int_str = {}
def translate():
    global mapping_int_str
    concordance_file = os.path.join(output_folder, f"{gt_type}/a{alpha}.concordance")
    # Change back to original names
    # cre")ate a integer to string mapping first
...
def extract_bucky_species_trees():
    ...
    if gt_type == 'all_in_bootstrap':
        output_population_tree_file = os.path.join(stree_output_folder, f"a{alpha}-bucky-RAxML.tre")
        output_concordance_tree_file = os.path.join(stree_output_folder, f"a{alpha}-bucky-conc-RAxML.tre")
    else:
        output_population_tree_file = os.path.join(stree_output_folder, f"a{alpha}-bucky-mrbayes.tre")
        output_concordance_tree_file = os.path.join(stree_output_folder, f"a{alpha}-bucky-conc-mrbayes.tre")
    ...
    for num in range(37, 0, -1):
        population_tree = population_tree.replace(str(num), mapping_int_str[num])
        concordance_tree = concordance_tree.replace(str(num), mapping_int_str[num])
...
def extract_bucky_species_quartets():
    ...
    concordance_file = os.path.join(output_folder, f"{gt_type}/a{alpha}.concordance")
    if gt_type == 'all_in_bootstrap':
        output_wqrts_file_wqfm = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-RAxML.wqrts")
        output_wqrts_file_wqmc = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-RAxML.wqmc.wqrts")
    else:
        output_wqrts_file_wqfm = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-mrbayes.wqrts")
        output_wqrts_file_wqmc = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-mrbayes.wqmc.wqrts")
...
def run_wqfm():
    ...
    if gt_type == 'all_in_bootstrap':
        input_wqrts_file = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-RAxML.wqrts")
        wQFM_tree_file = os.path.join(stree_output_folder, f"a{alpha}-wQFM-bucky-RAxML.tre")
    else:
        input_wqrts_file = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-mrbayes.wqrts")
        wQFM_tree_file = os.path.join(stree_output_folder, f"a{alpha}-wQFM-bucky-mrbayes.tre")

    temporary_tree_file = os.path.join(stree_output_folder, f"wQFM-bucky.temp")
    ...
    for number in range(37, 0, -1):
        tree = tree.replace(str(number), mapping_int_str[number])
...
def run_wqmc():
    if gt_type == 'all_in_bootstrap':
        input_wqrts_file = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-RAxML.wqmc.wqrts")
        output_tree_file = os.path.join(stree_output_folder, f"a{alpha}-wQMC-bucky-RAxML.tre")
    else:
        input_wqrts_file = os.path.join(wqrts_output_folder, f"a{alpha}-bucky-mrbayes.wqmc.wqrts")
        output_tree_file = os.path.join(stree_output_folder, f"a{alpha}-wQMC-bucky-mrbayes.tre")

    temporary_tree_file = os.path.join(stree_output_folder, f"wQMC-bucky.temp")
