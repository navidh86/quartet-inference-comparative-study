import os
import sys
ten_bucky = __import__("10_bucky_prep")
...
fp_fn_folder = f"{base_folder}/Estimated-Species-Trees/FP_FN/10-taxon/{configuration}"
rf_folder = f"{base_folder}/Estimated-Species-Trees/RF-rates/10-taxon/{configuration}"
...
def run_svd_wqmc():
	for mode in ["SVD-reciprocal", "SVD-exponential"]:
		for replicate_num in range(1, REPLICATES+1):
			...
			# create a two level map to eliminate ambiguity
			mapping_int_chr = {}
			mapping_chr_str = {}
			for num in range(1, 12):
				c = chr(ord('a') + num)
				mapping_int_chr[num] = c
				mapping_chr_str[c] = mapping_int_str[num]
			...
			for num in range(11, 0, -1):
				tree = tree.replace(str(num), mapping_int_chr[num])
			
			# to str now
			for num in range(1, 12):
				c = chr(ord('a') + num)
				tree = tree.replace(c, mapping_chr_str[c])
		...
		for gene in range(1, GENES+1):
			...
			with open(combined_gene_tree_file, "a") as fp:
				for line in lines[12:]:
					...
					# remap
					# needs to be remapped in two stages, we use the replace function in 10 bucky prep
					# for nmb in range(15, 0, -1):
						# tree = tree.replace(str(nmb), mapping_int_str[nmb])
					tree = ten_bucky.custom_replace(tree, mapping_int_str)
...
def run_wqmc_gtf(adjusted=False, boot=False, distance=False, dominant=False, bucky_boot=False):
	for replicate_num in range(1, REPLICATES+1):
		...
		if dominant:
			...
			# replace now
			replace_wqmc(input_dqrts_file, output_dqrts_file)
			# run wqmc
			exec_wqmc(output_dqrts_file, output_tree_file)
		...
		# replace now
		replace_wqmc(input_wqrts_file, output_wqrts_file)

		# run wqmc
		exec_wqmc(output_wqrts_file, output_tree_file)
...
def compute_fp_fn_rates(additional=None):
	...
	for mode in modes: # per model condition
		...
		for rep in range(1, REPLICATES+1): # per replicate
			model_tree_file = f"{base_folder}/10-taxon/{configuration}/true-speciestrees/R{rep}.true.tre"
