...
output_folder = f'{base_folder}/15-taxon/{configuration}/estimated-genetrees'
stree_output_folder = f"{base_folder}/Estimated-Species-Trees/15-taxon/{configuration}"
wqrts_output_folder = f'{base_folder}/15-taxon/Output/{configuration}'
...
ngen=55000
nruns=2
mbsumburnin=6 # should have been 11 :) 
...
def prepare_nexus_sequences():
    for replicate_num in range(1, REPLICATES+1):
        ...
        for gene in range(1, GENES+1):
            input_file_sequences_fasta = os.path.join(input_folder, f"R{replicate_num}/{gene}/{gene}.fasta")
...
def prepare_mr_bayes_commands():
    for replicate_num in range(1, REPLICATES+1):
        ...
        for gene in range(1, GENES+1):
            ...
            nexus_command_file = os.path.join(output_folder_rep, f"mb{gene}-s1000.nex")
...
def run_mr_bayes():
    ...
    for replicate_num in range(1, REPLICATES+1):
        ...
        for gene in range(1, GENES+1):
            cmd = f"mb mb{gene}-s1000.nex >> {gene}.log"
...
def run_mbsum():
    ...
    for replicate_num in range(1, REPLICATES+1):
        ...
        output_folder_rep = os.path.join(output_folder, f"R{replicate_num}/all_in_s1000")
...
def convert_to_numbers(tree):
    for nmb in range(ord('A'), ord('P')):
        tree = tree.replace(chr(nmb), str(nmb-ord('A')+1))
