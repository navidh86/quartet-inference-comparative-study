...
genes = 100 if len(sys.argv) < 2 else int(sys.argv[1])
# modes = [f'{genes}gene-100bp', f'{genes}gene-1000bp']
modes = [f'{genes}gene-1000bp', f'{genes}gene-100bp']
colors = ['orange', 'blue']
# linestyles = ['-', '--']
linestyles = ['--', '-']
alphas = [1, 1]
...
for idx, mode in enumerate(modes):
    ...
    # plt.plot(x, y_est, linestyle=linestyles[idx], marker='o', color=colors[0], alpha=alphas[idx])
    plt.plot(x, y_est, linestyle=linestyles[idx], color=colors[0], alpha=alphas[idx])
    ...
    # plt.plot(x, y_true, linestyle=linestyles[idx], marker='o', color=colors[1], alpha=alphas[idx])
    plt.plot(x, y_true, linestyle=linestyles[idx],color=colors[1], alpha=alphas[idx])
    for method_name in order:
...
# legend
lines = []
lines.append(Line2D([0,1], [0,1], color='black'))
lines.append(Line2D([0,1], [0,1], linestyle='dashed', color='black'))
lines.append(Line2D([], [], linestyle='')) # for gap
for color in colors:
    lines.append(Line2D([0,1],[0,1], color=color))    
...
out_file = f"{base_folder}/Results/figures/qscore/15taxon-{genes}gene.png"
