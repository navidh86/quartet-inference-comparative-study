...
mapping2 = {
    ...
    'wQFM-GTF-dominant-unweighted': 'wQFM-GTF-dominant (without weights)',
    'wQMC-GTF-dominant-unweighted': 'wQMC-GTF-dominant (without weights)'
}

# interest = ['wQFM-GTF', 'wQFM-GTF-dominant', 'wQFM-GTF-dominant-unweighted',
#             'wQMC-GTF', 'wQMC-GTF-dominant', 'wQMC-GTF-dominant-unweighted']

# order = ["wQFM-RAxML", "wQFM-GTF", "wQMC-RAxML", "wQMC-GTF"]
order = ["wQFM-GTF", "wQFM-GTF-boot", "wQMC-GTF", "wQMC-GTF-boot"]

order = [mapping2[x] if x in mapping2 else x for x in order]        
# hue_order = ['#a6bddb', '#74a9cf', '#a1d99b', '#74c476']
...
out_file = "/home/navidh86/Desktop/comparative_study/Alignments-WeightedQuartets-SpeciesTree-main/Results/figures/bar/" + f"RQ1-box-boot-{TAXA}-taxon" + ".pdf"
...
if TAXA == 10:
    ...
    plt.subplot(1, 2, 1)
    sns.boxplot(data=data.query('mode == "lower-ILS" and method in @order'), 
    ...
    plt.subplot(1, 2, 2)
    sns.boxplot(data=data.query('mode == "higher-ILS" and method in @order'), 
                x='method', y='rf', order=order, width=1, palette=hue_order, orient="v")
    plt.xlabel(None)
    plt.xticks([])
    plt.yticks(np.arange(0.0, ytick_lim, 0.1), fontsize=tick_fontsize)
