...
order = [] # won't be required
method_order = ["wQFM-GTF", "wQFM-GTF-boot", "wQFM-GTF-bucky-boot", "wQMC-GTF", "wQMC-GTF-boot",
         "wQMC-GTF-bucky-boot",]

method_order = [mapping2[x] if x in mapping2 else x for x in method_order]

# hue_order = ['#a6bddb', '#4292c6', '#08519c', '#a1d99b', '#41ab5d', '#006d2c']
hue_order = ['#7fc97f','#beaed4','#fdc086','#dddd33','#386cb0','#f0027f']
...
out_dir = f"{base_folder}/Results/figures/line"
...
if TAXA == 10:
    ...
    mode_order = ["100gene-100bp", "100gene-1000bp", "1000gene-100bp", "1000gene-1000bp"]
    ylim = 0.38
    ...
    if variety == "-ILS":
        ...
        ylim = 0.174
        ytick_lim = 0.19 # upto but not including
    else:
        ylim = 0.103
...
if TAXA == 10:
    ...
    f, ax = plt.subplots(1, 1, figsize=(20, 14), squeeze=True)
    
    plt.subplot(1, 1, 1)
    # figure(figsize=(15, 10))
    sns.pointplot(data=data, x="mode", y="rf", hue="method", order=mode_order, hue_order=method_order, palette=hue_order, errorbar=errorbar, capsize=capsize)
    # sns.barplot(data=data.query('mode == "100gene-100bp" and method in @order'), 
    #             x='method', y='rf', order=order, width=1, palette=hue_order, capsize=capsize, errorbar=errorbar)
    plt.xlabel("Model Condition", fontsize=ylabel_fontsize)
    # plt.xticks([])
    plt.yticks(np.arange(0.0, ytick_lim, 0.1), fontsize=tick_fontsize)
    plt.ylim(0.0, ylim)
    plt.ylabel("RF Rate", fontsize=ylabel_fontsize)
...
#         ba = [0.513, 0.07]
    ...
    legend_elements = []
    for idx, item in enumerate(method_order):
        line = Line2D([0], [0], color=hue_order[idx], marker='o', label=item)
        # patch = Rectangle((0, 0), width=0.5, height=2, label=item, color=hue_order[idx],
        #                 angle=45)
        legend_elements.append(line)

    plt.legend(handles=legend_elements, loc="upper right", fontsize=legend_fontsize)
